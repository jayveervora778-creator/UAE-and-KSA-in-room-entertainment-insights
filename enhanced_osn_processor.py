...
class EnhancedOSNProcessor:
    ...
    def _load_question_mapping(self):
        """Load question codes and actual question text from Excel headers"""
        try:
            # Load the header rows to build mapping
            df_codes = pd.read_excel(self.excel_path, sheet_name="Guests (UAE)", header=0, nrows=1)
            df_questions = pd.read_excel(self.excel_path, sheet_name="Guests (UAE)", header=1, nrows=1)
            
            # Build mapping from question codes to actual questions
            for i, (code, question) in enumerate(zip(df_codes.columns, df_questions.columns)):
